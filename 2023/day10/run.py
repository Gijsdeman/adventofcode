...
# file = open('day10/input', 'r').read().strip()
# lines = file.split('\n')
# sections = file.split('\n\n')
...
def part1(file):
    ...
    result, _ = createPipeCycle(grid)
    return result // 2


def part2floodfill(file):
    lines = file.split('\n')
    grid = [[char for char in row] for row in lines]
    grid = np.array(grid)
    rows = len(grid)
    ...
    _, pipeCycle = createPipeCycle(grid)
...
def part2shoelace(file):
    ...
    # Shoelace also counts border, which is not required
    return (abs(clockWise - counterClockWise) - pipeLength) / 2 + 1

def part2(file):
    # return part2floodfill(file)
    # return part2parity(file)
    return part2shoelace(file)